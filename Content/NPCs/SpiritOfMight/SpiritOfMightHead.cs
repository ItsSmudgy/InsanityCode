...
namespace Insanity.Content.NPCs.SpiritOfMight
{
	// The main part of the boss, usually refered to as "body"
	[AutoloadBossHead] // This attribute looks for a texture called "ClassName_Head_Boss" and automatically registers it as the NPC boss head icon
	public class SpiritOfMightHead : ModNPC
	{
		...
		public static int MinionType() {
			return ModContent.NPCType<MightProbe>();
		}
		...
		public override void SetDefaults() {
			...
			if (!Main.dedServ) {
				Music = MusicLoader.GetMusicSlot(Mod, "Assets/Music/spirit-of-might");
			}
		}
		...
		public override void SetBestiary(BestiaryDatabase database, BestiaryEntry bestiaryEntry) {
			...
			bestiaryEntry.Info.AddRange(new List<IBestiaryInfoElement> {
				...
				new FlavorTextBestiaryInfoElement("A being manifested from the power of souls of might. It is a very mighty foe.")
			});
		}
		...
		public override void ModifyNPCLoot(NPCLoot npcLoot) {
			...
			npcLoot.Add(ItemDropRule.BossBag(ModContent.ItemType<SpiritOfMightPetItem>()));
			npcLoot.Add(ItemDropRule.BossBag(ModContent.ItemType<SmudgyBall>()));
			...
			notExpertRule.OnSuccess(ItemDropRule.Common(ModContent.ItemType<SmudgyBall>(), 7));

			// This part is not required for a boss and is just showcasing some advanced stuff you can do with drop rules to control how items spawn
			// We make 12-15 ExampleItems spawn randomly in all directions, like the lunar pillar fragments. Hereby we need the DropOneByOne rule,
			// which requires these parameters to be defined
			int itemType = ModContent.ItemType<Essence_of_Might>();
		...
		public override void HitEffect(int hitDirection, double damage) {
			...
			if (NPC.life <= 0) {
				...
				int backGoreType = GoreID.PirateShip9;
				int frontGoreType = GoreID.PirateShip10;
		...
		private void SpawnMinions() {
			...
			for (int i = 0; i < count; i++) {
				int index = NPC.NewNPC(entitySource, (int)NPC.Center.X, (int)NPC.Center.Y, ModContent.NPCType<MightProbe>(), NPC.whoAmI);
				...
				if (minionNPC.ModNPC is MightProbe minion) {
		...
		private void CheckSecondStage() {
			...
			for (int i = 0; i < Main.maxNPCs; i++) {
				...
				if (otherNPC.active && otherNPC.type == MinionType() && otherNPC.ModNPC is MightProbe minion) {
		...
		private void DoSecondStage(Player player) {
			...
			if (NPC.direction == -1 && NPC.Center.X - changeDirOffset < abovePlayer.X ||
				NPC.direction == 1 && NPC.Center.Y + changeDirOffset > abovePlayer.Y ||
